...
if __name__ == '__main__':
    ...
    for idx, plt_disp in enumerate(pred_disparities):
        ...
        if idx % 10 == 0:
            ...
            # plt.imsave(f"visualization/evaluate/{args.exp_name}/plt_{idx}.png", plt_disp, cmap='plasma')
    ...
    if args.split == 'kitti':
        ...
        gt_disparities_noc = load_gt_disp_kitti(args.gt_path, 'noc')
    ...
    epe_all = np.zeros(num_samples, np.float32)
    epe_noc = np.zeros(num_samples, np.float32)
    epe_occ = np.zeros(num_samples, np.float32)
    ...
    for i in range(num_samples):
        ...
        if args.split == 'kitti':
            ...
            gt_disp_noc = gt_disparities_noc[i]
            mask = gt_disp > 0
            noc_mask = gt_disp_noc > 0
            pred_disp = pred_disparities_resized[i]

            disp_diff = np.abs(gt_disp[mask] - pred_disp[mask]) # epe_all
            bad_pixels = np.logical_and(disp_diff >= 3, (disp_diff / gt_disp[mask]) >= 0.05)
            d1_all[i] = 100.0 * bad_pixels.sum() / mask.sum()

        abs_rel[i], sq_rel[i], rms[i], log_rms[i], a1[i], a2[i], a3[i] = compute_errors(gt_depth[mask], pred_depth[mask])
        if args.split == 'kitti':
            epe_all[i], epe_noc[i], epe_occ[i] = compute_epe(np.sqrt(np.square(gt_disp - pred_disp)), mask, noc_mask)
    ...
    if args.split == 'eigen':
        ...
        print("{:>8}, {:>8}, {:>8}, {:>8}, {:>8}, {:>8}, {:>8}, {:>8}, {:>8}, {:>8}, {:>8}".format('abs_rel', 'sq_rel', 'rms', 'log_rms', 'd1_all', 'a1', 'a2', 'a3', 'epe_noc', 'epe_all', 'epe_occ'))
        print("{:8.4f}, {:8.4f}, {:8.3f}, {:8.3f}, {:8.3f}, {:8.3f}, {:8.3f}, {:8.3f}, {:8.3f}, {:8.3f}, {:8.3f}".format(abs_rel.mean(), sq_rel.mean(), rms.mean(), log_rms.mean(), d1_all.mean(), a1.mean(), a2.mean(), a3.mean(), epe_noc.mean(), epe_all.mean(), epe_occ.mean()))
