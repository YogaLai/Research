...
for epoch in range(args.num_epochs):
    ...
    for batch_idx, (left_image_1, left_image_2, right_image_1, right_image_2) in enumerate(CycleLoader):
        ...
        for i in range(args.num_scales):
            ...
            fw_mask += 1e-3
            bw_mask += 1e-3
            fw_mask[:args.batch_size] = fw_mask[:args.batch_size] * 0 + 1
            fw_mask[-args.batch_size:] = fw_mask[-args.batch_size:] * 0 + 1
            bw_mask[:args.batch_size] = bw_mask[:args.batch_size] * 0 + 1
            bw_mask[-args.batch_size:] = bw_mask[-args.batch_size:] * 0 + 1
            
            #reconstruction from right to left
            left_est = flow_warp(right_pyramid, disp_est_scale[i])
            l1_left = torch.abs(left_est - left_pyramid) * fw_mask
            l1_reconstruction_loss_left = torch.mean(l1_left) / torch.mean(fw_mask) 
            ssim_left = SSIM(left_est * fw_mask, left_pyramid * fw_mask) 
        ...
        if args.type_of_2warp == 1:
            ...
            warp2_est_4 = [flow_warp(left_est[i][[0,1]], disp_est_scale[i][[2,3]]) for i in range(4)]
            loss += 0.1 * sum([warp_2(warp2_est_4[i], left_pyramid[i][[6,7]], mask_4[i], args) for i in range(4)])
            mask_5 = [bw_mask[i][[2,3]] for i in range(4)]
            warp2_est_5 = [flow_warp(left_est[i][[6,7]], disp_est_scale_2[i][[2,3]]) for i in range(4)]
