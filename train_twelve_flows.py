...
def get_args():
    ...
    parser.add_argument('--census_loss_weight',        type=float, help='census loss weight', default=1)
...
def train(epoch, dataloader, net, optimizer, scheduler, writer, args):
    ...
    with tqdm(total=len(dataloader.dataset)) as pbar:
        for batch_idx, (left_image_1, left_image_2, right_image_1, right_image_2) in enumerate(dataloader):
            ...
            former = torch.cat((left_image_2, left_image_1, right_image_1,  left_image_1, right_image_1, left_image_1), 0).cuda()
            latter = torch.cat((right_image_2, left_image_2, right_image_2,  right_image_2, left_image_2, right_image_1), 0).cuda()
            ...
            if args.use_boundary_dilated_warp:
                ...
                fw, bw, diff_fw, diff_bw = get_mix_mask(disp_est_scale, disp_est_scale_2, border_mask, slices[1]+slices[2]+slices[3]+slices[4])
            if epoch < 2:
                fw += 1e-3
                bw += 1e-3
