...
def bcn(in_planes, out_planes, kernel_size=3, stride=1, padding=1, dilation=1):   
    return nn.Sequential(
            nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation, groups=1, bias=False),
...
class FPNExtractor(nn.Module):
    def __init__(self):
        ...
        self.conv1a = bcn(3,   16, kernel_size=3, stride=2)
        self.conv1b = bcn(16,  16, kernel_size=3, stride=1)
        self.conv2a = bcn(16,  32, kernel_size=3, stride=2)
        self.conv2b = bcn(32,  32, kernel_size=3, stride=1)
        self.conv3a = bcn(32,  64, kernel_size=3, stride=2)
        self.conv3b = bcn(64,  64, kernel_size=3, stride=1)
        self.conv4a = bcn(64,  96, kernel_size=3, stride=2)
        self.conv4b = bcn(96,  96, kernel_size=3, stride=1)
        self.conv5a = bcn(96, 128, kernel_size=3, stride=2)
        self.conv5b = bcn(128,128, kernel_size=3, stride=1)
        self.conv6a = bcn(128,196, kernel_size=3, stride=2)
        self.conv6b = bcn(196,196, kernel_size=3, stride=1)

        # laterial layers
        self.conv_out2 = nn.Conv2d(32, 32, kernel_size=1, padding=0)
        self.conv_out3 = nn.Conv2d(64, 32, kernel_size=1, padding=0)
        self.conv_out4 = nn.Conv2d(96, 32, kernel_size=1, padding=0)
        self.conv_out5 = nn.Conv2d(128, 32, kernel_size=1, padding=0)
        self.conv_out6 = nn.Conv2d(196, 32, kernel_size=1, padding=0)

        self.smooth2 = nn.Conv2d(32, 32, kernel_size=3, padding=1)
        self.smooth3 = nn.Conv2d(32, 32, kernel_size=3, padding=1)
        self.smooth4 = nn.Conv2d(32, 32, kernel_size=3, padding=1)
        self.smooth5 = nn.Conv2d(32, 32, kernel_size=3, padding=1)
    ...
    def forward(self, img):
        f11 = self.conv1b(self.conv1a(img))
        f12 = self.conv2b(self.conv2a(f11))
        f13 = self.conv3b(self.conv3a(f12))
        f14 = self.conv4b(self.conv4a(f13))
        f15 = self.conv5b(self.conv5a(f14))
        f16 = self.conv6b(self.conv6a(f15))

        c16 = self.conv_out6(f16)
        c15 = self.upsample_add(c16, self.conv_out5(f15))
        c14 = self.upsample_add(c15, self.conv_out4(f14))
        c13 = self.upsample_add(c14, self.conv_out3(f13))
        c12 = self.upsample_add(c13, self.conv_out2(f12))
...
class PWCDCNet(nn.Module):
    ...
    def __init__(self, md=4):
        ...
        self.feature_extractor = FPNExtractor()
    ...
    def forward(self,x):
        ...
        c12, c13, c14, c15, c16 = self.feature_extractor(im1)
        c22, c23, c24, c25, c26 = self.feature_extractor(im2)
