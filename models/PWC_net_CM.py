...
class PWCDCNet(nn.Module):
    ...
    def __init__(self, md=4):
        ...
        #     nn.Conv2d(96,1, kernel_size=5, padding=2),
        #     nn.Sigmoid()
        # )
        ...
        # self.modulate4 = torch.nn.Sequential(
        #     torch.nn.Conv2d(in_channels=96+81+1, out_channels=128, kernel_size=3, stride=1, padding=1),
        #     torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
        #     torch.nn.Conv2d(in_channels=128, out_channels=64, kernel_size=3, stride=1, padding=1),
        #     torch.nn.LeakyReLU(inplace=False, negative_slope=0.1)
        # )
        self.modulate3 = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=64+81+1, out_channels=128, kernel_size=3, stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
            torch.nn.Conv2d(in_channels=128, out_channels=64, kernel_size=3, stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1)
        )
        self.modulate2 = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=32+81+1, out_channels=128, kernel_size=3, stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
            torch.nn.Conv2d(in_channels=128, out_channels=64, kernel_size=3, stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1)
        )
        self.corr_scalar = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=64, out_channels=32, kernel_size=3, stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
            torch.nn.Conv2d(in_channels=32, out_channels=81, kernel_size=1, stride=1, padding=0)
        )
        self.corr_offset = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=64, out_channels=32, kernel_size=3, stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
            torch.nn.Conv2d(in_channels=32, out_channels=81, kernel_size=1, stride=1, padding=0)
        )
    ...
    def forward(self,x):
        ...
        # feat_diff = (c14-warp4).pow(2).sum(1,True).sqrt()
        # conf4 = self.sigmoid(feat_diff)
        # corrfeat = self.modulate4(torch.cat([c14, corr4, conf4], 1))
        # corr_scalar = self.corr_scalar(corrfeat)
        # corr_offset = self.corr_offset(corrfeat)
        # corr4 = corr_scalar * corr4 + corr_offset
        ...
        # feat_diff = (c13-warp3).pow(2).sum(1,True).sqrt() # training is not work
        feat_diff = (c13-warp3).abs()
        conf3 = self.conf3(feat_diff)
        corrfeat = self.modulate3(torch.cat([c13, corr3, conf3], 1))
        corr_scalar = self.corr_scalar(corrfeat)
        corr_offset = self.corr_offset(corrfeat)
        corr3 = corr_scalar * corr3 + corr_offset
        ...
        # feat_diff = (c12-warp2).pow(2).sum(1,True).sqrt()
        # conf2 = self.sigmoid(feat_diff)
        feat_diff = (c12-warp2).abs()
        conf2 = self.conf2(feat_diff)
        corrfeat = self.modulate2(torch.cat([c12, corr2, conf2], 1))
        corr_scalar = self.corr_scalar(corrfeat)
        corr_offset = self.corr_offset(corrfeat)
        corr2 = corr_scalar * corr2 + corr_offset
