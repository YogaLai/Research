...
class PWCDCNet(nn.Module):
    ...
    def __init__(self, md=3, attn_match=False):
        ...
        self.dc_conv1 = myconv(34, 128, kernel_size=3, stride=1, padding=1,  dilation=1)
    ...
    def forward(self,x):
        ...
        if self.training: 
            maybe_dropout = torch.tensor(random.uniform(0,1) > self.dropout_rate, dtype=torch.float, device=x.device) 
            # x *= maybe_dropout # dropout upfeat
            flow5 *= maybe_dropout
        flow5 += up_flow6
