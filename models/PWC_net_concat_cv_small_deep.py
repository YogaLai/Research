...
from utils.correltaiton import MatchingNetDeep, MatchingNetSmallAttn, compute_cost
...
class PWCDCNet(nn.Module):
    ...
    def __init__(self, md=3, attn_match=False, attn_dap=False, dcn=False):
        ...
        if attn_match:
            ...
            self.matchnet2 = MatchingNetDeep(dcn=dcn)
            self.matchnet3 = MatchingNetDeep(dcn=dcn)
            self.matchnet4 = MatchingNetDeep(dcn=dcn)
            self.matchnet5 = MatchingNetDeep(dcn=dcn)
            self.matchnet6 = MatchingNetDeep(dcn=dcn)
