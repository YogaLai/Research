...
def main_worker(rank, n_GPUs):
    ...
    trainset = myCycleImageFolder(left_image_1, left_image_2, right_image_1, right_image_2, True, args)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        with tqdm(total=len(CycleLoader.dataset)) as pbar:
            for batch_idx, (left_image_1, left_image_2, right_image_1, right_image_2) in enumerate(CycleLoader):
                ...
                model_input = torch.cat((former, latter), 1).to(rank)
                model_input_2 = torch.cat((latter, former), 1).to(rank)
